...
func (a *App) GetOAuthLoginEndpoint(w http.ResponseWriter, r *http.Request, service, teamID, action, redirectTo, loginHint string, isMobile bool) (string, *model.AppError) {
	fmt.Println("************************ GetOAuthLoginEndpoint App/oauth.go *****************************")

	stateProps := map[string]string{}
	stateProps["action"] = action
	fmt.Println("_____ action", action)

	if teamID != "" {
		stateProps["team_id"] = teamID
	}

	fmt.Println(" _________ team_id: ", teamID)
	if redirectTo != "" {
		stateProps["redirect_to"] = redirectTo
	}

	fmt.Println("_____ strconv.FormatBool(isMobile)", strconv.FormatBool(isMobile)) // probably false
	...
	fmt.Println("_____ authURL", authURL)
	if err != nil {
		return "", err
	}
...
func (a *App) GetOAuthSignupEndpoint(w http.ResponseWriter, r *http.Request, service, teamID string) (string, *model.AppError) {
	fmt.Println("************************ GetOAuthSignupEndpoint App *****************************")
...
func (a *App) getSSOProvider(service string) (einterfaces.OAuthProvider, *model.AppError) {
	fmt.Println("******************************** getSSOProvider ********************************")

	sso := a.Config().GetSSOService(service)

	fmt.Println(" _______ SSO: ", sso.UserAPIEndpoint)
	...
	fmt.Println("_______ SSO.scope: ", *sso.Scope)
	fmt.Println("_______ OpenIDScope: ", OpenIDScope)

	if strings.Contains(*sso.Scope, OpenIDScope) {
		providerType = model.ServiceOpenid
	}

	fmt.Println("_______ providerType: ", providerType)
...
func (a *App) LoginByOAuth(c *request.Context, service string, userData io.Reader, teamID string, tokenUser *model.User) (*model.User, *model.AppError) {
	fmt.Println("******************************** LoginByOAuth app/oauth.go ********************************")
...
func (a *App) GetAuthorizationCode(w http.ResponseWriter, r *http.Request, service string, props map[string]string, loginHint string) (string, *model.AppError) {
	fmt.Println("******************************** GetAuthorizationCode ********************************")

	provider, e := a.getSSOProvider(service)
	fmt.Println("_____ provider", provider)
	if e != nil { ///////GOOGLE ISSUE OCCUR HERE
		return "", e
	}

	sso, e2 := provider.GetSSOSettings(a.Config(), service)
	fmt.Println("_____ sso", sso.Id)
	//fmt.Println("_____ e2", e2.)
	...
	fmt.Println("_____ props[email]", props["email"])

	tokenExtra := generateOAuthStateTokenExtra(props["email"], props["action"], cookieValue)
	fmt.Println("_____ tokenExtra]", tokenExtra)
	...
	fmt.Println("_____ siteURL", siteURL)
...
func (a *App) AuthorizeOAuthUser(w http.ResponseWriter, r *http.Request, service, code, state, redirectURI string) (io.ReadCloser, string, map[string]string, *model.User, *model.AppError) {
	fmt.Println("******************************** AuthorizeOAuthUser ********************************")

	provider, e := a.getSSOProvider(service)
	fmt.Println("_______ 1")
	if e != nil {
		return nil, "", nil, nil, e
	}

	sso, e2 := provider.GetSSOSettings(a.Config(), service)
	fmt.Println("_______ 2")
	...
	//js , err := json
	fmt.Println("_______ 1st resp.Body", resp)
	...
	fmt.Println("___stateProps: ", stateProps)
	...
	fmt.Println("_______ar.IdToken ", ar.IdToken)
	fmt.Println("_______ar.AccessToken ", ar.AccessToken)
	fmt.Println("_______ar.RefreshToken ", ar.RefreshToken)

	fmt.Println("_______ar.Scope ", ar.Scope)
	fmt.Println("_______ar.ExpiresInSeconds ", ar.ExpiresInSeconds)
	fmt.Println("_______ar.TokenType ", ar.TokenType)
	...
	fmt.Println("_______ *sso.UserAPIEndpoint: ", *sso.UserAPIEndpoint)
	////////////////////////////////////////////////////////////////////////////////////////////////
	req, requestErr = http.NewRequest("GET", *sso.UserAPIEndpoint, strings.NewReader(""))
	fmt.Println("_______ 1", req.Response)
	...
	s, _ := json.Marshal(resp.Body)
	fmt.Println("=== resp.Body: ", string(s))
	fmt.Println("==== resp.Body ", resp.Body)
	fmt.Println("==== teamID ", teamID)
	fmt.Println("=====stateProps ", stateProps)
	fmt.Println("==== userFromToken ", userFromToken)
